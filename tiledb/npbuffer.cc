...
bool issubdtype(nb::dlpack::dtype t1, nb::dlpack::dtype t2) {
...
template <typename T>
nb::dlpack::dtype get_dtype(T obj) {
    auto& api = nb::detail::npy_api::get();

    if (api.PyArray_Check_(obj.ptr())) {
        return nb::cast<nb::ndarray>(obj).dtype();
    }

    return nb::steal<nb::dlpack::dtype>(
...
//       not what we want.
bool dtype_equal(nb::dlpack::dtype d1, nb::dlpack::dtype d2) {
    auto& api = nb::detail::npy_api::get();
...
class NumpyConvert {
   private:
    ...
    void convert_unicode() {
        ...
        // NOTE: NumPy fixed-length string arrays *do not support* embedded
        // nulls.
        //       There is no string size stored, so string end is demarcated by
        //       \0 and the slot is filled to the next boundary with \0.
        // For consistency and to avoid complications in other APIs, we are
        // storing all string arrays as var-length.
        ...
        auto npstrencode = nb::module_::import_("numpy").attr("str_").attr(
            "encode");
    ...
    void convert_bytes() {
        ...
        // avoid one interpreter roundtrip
        // auto npstrencode =
        // nb::module_::import_("numpy").attr("str_").attr("encode");
    ...
    void convert_object() {
            ...
            if (PyUnicode_Check(o)) {
                if (!allow_unicode_) {
                    ...
                    auto errmsg = std::string(
                        "Unexpected unicode object for TILEDB_STRING_ASCII "
                        "attribute");
                ...
                if (!input_p) {
                    TPY_ERROR_LOC(
                        "Internal error: failed to convert unicode to UTF-8");
                }
            } else if (PyBytes_Check(o)) {
                // ASCII only
                auto res = PyBytes_AsStringAndSize(
                ...
                if (res == -1) {
                    // TODO TPY_ERROR_LOC
                    throw std::runtime_error(
                        "Internal error: failed to get char* from bytes "
                        "object");
                }
        ...
        for (int64_t idx = 0; idx < input_size; idx++) {
            ...
            if (PyUnicode_Check(pyobj_p)) {
                ...
                auto arr = nb::cast<nb::ndarray>(pyobj_p);
                sz = arr.nbytes();
    ...
    void convert_iter() {
        ...
        for (auto obj_h : iter) {
            ...
            // we must check each dtype because object arrays are not guaranteed
            // to be homogenous
            auto cur_dtype = get_dtype(obj_h);
            auto err_str = std::string(
                "Mismatched element type in buffer conversion!");
            ...
            if (PyUnicode_Check(obj_p)) {
                ...
                // ASCII only
                auto res = PyBytes_AsStringAndSize(
                    obj_p, const_cast<char**>(&input_p), &sz);
                ...
                // handle (potentially) var-len embedded arrays
                sz = nb::cast<nb::ndarray>(obj_p).nbytes();
        ...
        for (auto obj_h : iter) {
            ...
            if (PyUnicode_Check(obj_p)) {
                ...
                auto o_a = nb::cast<nb::ndarray>(obj_h);
                sz = o_a.nbytes();
  ...
  */
    ...
    /*
    Returns a tuple of nb::ndarray containing
  ...
  */
    nb::tuple get() {
        ...
        auto data_np = nb::ndarray<uint8_t>(
            tmp_data_buf_p->size(), tmp_data_buf_p->data(), data_ref);
        auto offset_np = nb::ndarray<uint64_t>(
...
nb::tuple convert_np(
    nb::ndarray input, bool allow_unicode, bool use_fallback = false) {
    if (use_fallback) {
        auto tiledb = nb::module_::import_("tiledb");
