...
class SparseArray(DiskTestCase):
    ...
    def test_query_specified_dim_coords(self):
        uri = self.path("query_specified_dim_coords")

        ctx = tiledb.Ctx()
        dom = tiledb.Domain(
                tiledb.Dim("i", domain=(1, 10), tile=10, dtype=int, ctx=ctx),
                tiledb.Dim("j", domain=(11, 20), tile=10, dtype=int, ctx=ctx),
                ctx=ctx)
        att_x = tiledb.Attr("x", dtype=int, ctx=ctx)
        att_y = tiledb.Attr("y", dtype=float, ctx=ctx)
        schema = tiledb.ArraySchema(domain=dom, attrs=(att_x, att_y), sparse=True, ctx=ctx)
        tiledb.SparseArray.create(uri, schema)

        i = np.array([1, 1, 1, 2, 3, 3, 3, 4])
        j = np.array([11, 12, 14, 13, 11, 16, 17, 15])

        x = np.array([0, 1, 2, 3, 4, 6, 7, 5])
        y = np.array([0, .1, .2, .3, .4, .6, .7, .5])
