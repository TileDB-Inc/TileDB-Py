...
class DimensionLabelSchema {
   public:
    ...
    tiledb_datatype_t dim_type() const {
        return dim_type_;
    }
    ...
    const void* dim_tile_extent() const {
        ...
        nb::buffer tile_buffer = nb::buffer(dim_tile_extent_);
        nb::buffer_info tile_extent_info = tile_buffer.request();
        return tile_extent_info.ptr;
    }

    bool has_dim_tile_extent() const {
        return !dim_tile_extent_.is_none();
    }

    bool has_label_filters() const {
        return label_filters_.has_value();
    }
...
void init_schema(nb::module_& m) {
    nb::class_<DimensionLabelSchema>(m, "DimensionLabelSchema")
        ...
        .def_prop_rw_readonly("_dim_dtype", &DimensionLabelSchema::dim_type)
        ...
        .def_prop_rw_readonly(
            "_label_order", &DimensionLabelSchema::label_order)

        .def_prop_rw_readonly(
            "_dim_tile_extent",
            [](DimensionLabelSchema& dim_label_schema) -> nb::object {
                ...
                switch (dim_type) {
                    case TILEDB_UINT64: {
                        using T = uint64_t;
                        return nb::cast(*static_cast<const T*>(tile_extent));
                    }
                    ...
                    case TILEDB_STRING_ASCII: {
                        // This should have already been caught in the check for
                        // a mullptr above.
                        throw TileDBError(
                            "Setting a tile extent on a string dimension "
                            "is not supported for dimension labels");
                    }
                    default:
                        throw TileDBError(
                            "Unsupported dtype for dimension tile extent");
                }
            });

    nb::class_<tiledb::ArraySchema>(m, "ArraySchema")
        .def(nb::init<ArraySchema>())

        .def(nb::init<Context&, tiledb_array_type_t>(), nb::keep_alive<1, 2>())

        .def(nb::init<Context&, std::string&>())

        .def(nb::init<Context&, nb::capsule>())

        .def(
            "__capsule__",
            [](ArraySchema& schema) {
                return nb::capsule(schema.ptr().get(), "schema");
            })
        ...
        .def_prop_rw("_domain", &ArraySchema::domain, &ArraySchema::set_domain)

        .def_prop_rw_readonly("_array_type", &ArraySchema::array_type)

        //  .def_prop_rw_readonly("timestamp_range",
        //  &ArraySchema::timestamp_range)

        .def_prop_rw(
            "_capacity", &ArraySchema::capacity, &ArraySchema::set_capacity)

        .def_prop_rw_readonly("_version", &ArraySchema::version)

        .def_prop_rw(
            "_cell_order",
            &ArraySchema::cell_order,
        ...
        .def(
            ...
            nb::overload_cast<const std::string&>(
                &ArraySchema::attribute, nb::const_))
        .def(
            "_attr",
        ...
        .def(
            ...
            [](const ArraySchema& schema,
               ...
               const std::string& name) {
                return ArraySchemaExperimental::dimension_label(
                    context, schema, name);
            })

        .def_prop_rw_readonly("_nattr", &ArraySchema::attribute_num)
            ...
            [](ArraySchema& schema,
               ...
               const DimensionLabelSchema& dim_label_schema) {
                ...
                if (dim_label_schema.dim_type() != dim_type) {
                    throw TileDBError(
                        "Cannot add dimension label '" + name +
                ...
                if (dim_label_schema.has_dim_tile_extent()) {
                    ctx.handle_error(
                        tiledb_array_schema_set_dimension_label_tile_extent(
                            ...
                            name.c_str(),
                            dim_type,
                            dim_label_schema.dim_tile_extent()));
                }
            })
