...
void init_query(py::module& m) {
      py::class_<tiledb::Query>(m, "Query")
        ...
        //    tiledb_datatype_t dim_type = schema.domain().dimension(dim_idx).type();
        //    size_t dim_size = tiledb_datatype_size(dim_type);
        //    auto nptype =
        //})
        // TODO generic add_range implemented here
        //.def("set_data_buffer",
        //     (Query& (Query::*)(const std::string&, void*, uint64_t))&Query::set_data_buffer);
        .def("set_data_buffer", [](Query& q, std::string name, py::array a){
